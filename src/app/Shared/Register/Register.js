...
const Register = () => {
  ...
  const validateForm = () => {
    ...
    if (mobile === "") {
      ...
      setMobileError("mobile should be at least 10 characters long");
      valid = false;
    } else {
      setMobileError("");
    }
      ...
      axios
        ...
        .then((response) => {
          if (response.data.status === "success") {
            NotificationManager.success("Success message", "OTP Sent!");
  ...
  const handleConfirmPassword = (e) => {
    if (password === passwordConfirmation && password.length > 0) {
      ...
      if (currMobile?.length < 10) {
        NotificationManager?.warning("", "Please enter mobile");
        return;
      }
      handleVerifyNumber();
    }
  ...
  const handleSubmit = (event) => {
    ...
    if (!isChecked) {
      NotificationManager.warning("Please accept the terms and conditions");
      return;
    }
    if (!validateForm()) {
      return;
    ...
    try {
      axios
        .post("https://admin.leafymango.com/web/register", userData)
        .then((response) => {
          ...
          <form className="register-form-os" onSubmit={handleSubmit}>
            ...
            <div className="register-checkbox-os">
              <label className="container_checkbox-os">
                By creating your account, you agree to VIP NUMBER SHOP's Privacy
